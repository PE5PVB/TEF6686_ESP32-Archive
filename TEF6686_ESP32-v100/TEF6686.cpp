...
uint8_t TEF6686::init() {
  ...
  Tuner_I2C_Init();
  while (true) {
    result = devTEF_APPL_Get_Operation_Status(&status);
    if (result == 1) {
...
void TEF6686::powerOn() {
  devTEF_APPL_Set_OperationMode(0);
}

void TEF6686::powerOff() {
  devTEF_APPL_Set_OperationMode(1);
}
...
uint16_t TEF6686::tuneUp(uint16_t LowEdge, uint16_t HighEdge) {
  return tune(1, LowEdge, HighEdge);
}

uint16_t TEF6686::tuneDown(uint16_t LowEdge, uint16_t HighEdge) {
  return tune(0, LowEdge, HighEdge);
}
...
uint16_t TEF6686::seek(uint8_t up, uint16_t LowEdge, uint16_t HighEdge, uint16_t threshold) {
  ...
  while (true) {
    switch(mode){
      case 20:
        Radio_ChangeFreqOneStep(up, LowEdge, HighEdge);
...
uint16_t TEF6686::tune(uint8_t up, uint16_t LowEdge, uint16_t HighEdge) {
  Radio_ChangeFreqOneStep(up, LowEdge, HighEdge);
  Radio_SetFreq(Radio_GetCurrentFreq(), LowEdge, HighEdge);
