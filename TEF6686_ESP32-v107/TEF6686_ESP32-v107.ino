...
#define ORIENTATION     0 // Change to 1 to flip the screen
...
void setup() {
  ...
  if (EEPROM.readByte(39) != 5) {
    ...
    EEPROM.writeByte(39, 5);
    EEPROM.writeByte(40, 0);
    EEPROM.writeByte(41, 20);
    EEPROM.writeByte(42, 1);
  ...
  LowLevelSet = EEPROM.readByte(41);
  iMSset = EEPROM.readByte(42);
  EQset = EEPROM.readByte(43);
  EEPROM.commit();
  encoder.begin();   
  btStop();
  Serial.begin(115200);
...
void loop() {
  ...
  if (power == true) {
      if (SStatus/10 > LowLevelSet && LowLevelInit == false && menu == false) {
          radio.clearRDS();
      ...
      if (SStatus/10 <= LowLevelSet) {
        ...
        if (lowsignalmillis > lowsignaltimer + 500) {
          lowsignaltimer += 500;
    ...
    {
      radio.getStatus(SStatus, USN, WAM, OStatus, BW, MStatus);
      if (lowsignalmillis > lowsignaltimer + 500 || change2 == true) {
        lowsignaltimer += 500;
        change2 = false;
...
void ModeButtonPress() {
  if (menu == false) {
    ...
    EEPROM.writeByte(41, LowLevelSet);
    EEPROM.commit();
  }
...
void ShowStepSize() {
  ...
  if (stepsize == 3) {tft.fillRect(148, 38,15,4,TFT_GREEN);}
}

void RoundStep() {
  int freq = radio.getFrequency();
  if (freq%10 < 3) {radio.setFrequency(freq - (freq%10 - 5) -5, LowEdgeSet, HighEdgeSet);}
  else if (freq%10 > 2 && freq%10 < 8) {radio.setFrequency(freq - (freq%10 - 5) , LowEdgeSet, HighEdgeSet);}
  else if (freq%10 > 7) {radio.setFrequency(freq - (freq%10 - 5) +5, LowEdgeSet, HighEdgeSet);}
...
void ButtonPress() {
  if (menu == false) {
    ...
    EEPROM.writeByte(42, iMSset);
    EEPROM.writeByte(43, EQset);
    EEPROM.commit();
    }
  } else {
    if (menuopen == false) {
...
void KeyDown() {
    if (menu == false) {
      ...
      if (menuopen == false) {
        tft.drawRoundRect(10,menuoption,300,18,5,TFT_BLACK);
        menuoption -= 20;
        if (menuoption < 30) {menuoption = 210;}
        tft.drawRoundRect(10,menuoption,300,18,5,TFT_WHITE);
      } else {
        switch (menuoption) {
          case 30:
...
{
  if (SStatus > (SStatusold +50) || SStatus < (SStatusold -50))
  {
    String count = String(SStatus/10, DEC);
    ...
    if (SStatusold >= 0) {
      ...
      if (SStatusoldcount <= 1) {tft.setCursor (267, 110);}
      if (SStatusoldcount == 2) {tft.setCursor (250, 110);}
      if (SStatusoldcount >= 3) {tft.setCursor (223, 110);}
    }
    tft.setTextFont(6);
    tft.print(SStatusold/10);
    tft.setTextFont(4);
    ...
    if (SStatus >= 0) {
      ...
      if (count.length() == 1) {tft.setCursor (267, 110);}
      if (count.length() == 2) {tft.setCursor (250, 110);}
      if (count.length() >= 3) {tft.setCursor (223, 110);}
    }
    tft.setTextFont(6);
    tft.print(SStatus/10);
    tft.setTextFont(4);
...
void ShowOffset()
{
  if (OStatus != OStatusold) {
    ...
    if (OStatus > -250 && OStatus < 250)
    {
      tft.fillTriangle(6,8,6,22,14,14,TFT_GREYOUT);
      tft.fillTriangle(18,8,18,22,26,14,TFT_GREYOUT);
      if (SQ == false) {tft.fillCircle(32,15,3,TFT_GREEN);} else {tft.fillCircle(32,15,3,TFT_GREYOUT);}
      tft.fillTriangle(38,14,46,8,46,22,TFT_GREYOUT);
      tft.fillTriangle(50,14,58,8,58,22,TFT_GREYOUT);
    }
    if (OStatus > 250 && OStatus < 500)
      ...
      switch (buff[0])
      {
        ...
        case 'C':
          ...
          if (scanmethod == 1) {
            ...
            ShowFreq(1);
            radio.setMute();
            frequency = radio.seekUp(80, 110, Squelch);
            ShowFreq(3);
            ShowFreq(0);
            radio.setUnMute();
          }
          if (scanmethod == 2) {
            ...
            ShowFreq(2);
            radio.setMute();
            frequency = radio.seekDown(80, 110, Squelch);
            ShowFreq(4);
            ShowFreq(0);
            radio.setUnMute();
          }
