...
{
  return devTEF_Set_Cmd(TEF_FM,Cmd_Set_MphSuppression,5,mph);
}

uint8_t devTEF_Radio_Set_ChannelEqualizer(uint16_t eq)
{
  return devTEF_Set_Cmd(TEF_FM,Cmd_Set_ChannelEqualizer,5,eq);
}
...
{
  return devTEF_Set_Cmd(TEF_FM,Cmd_Set_Deemphasis,5,timeconstant);
}
